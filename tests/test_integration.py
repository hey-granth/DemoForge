...
class TestIntegration:
    ...
    async def test_full_execution_flow(self, mock_components):
        ...
        elem1 = InteractionElement("btn1", "button", "Features", "button", "", True, True)
        elem2 = InteractionElement("btn2", "button", "About", "button", "", True, True)
        
        discovery.scan_elements = AsyncMock(side_effect=[
            [elem1, elem2],
            [elem2],
            []
        ])
        discovery.filter_unvisited = MagicMock(side_effect=[
            [elem1, elem2],
            [elem2],
            []
        ])
        ...
        planner.rank_interactions = AsyncMock(side_effect=[
            [plan1],
            [plan2],
            []
        ])
    ...
    async def test_blacklist_prevents_dangerous_actions(self, mock_components):
        ...
        safe_elem = InteractionElement("btn1", "button", "Learn More", "button", "", True, True)
        danger_elem = InteractionElement("btn2", "button", "Delete Account", "button", "", True, True)
    ...
    async def test_concurrent_job_execution(self):
        async def run_job(job_id):
            browser = AsyncMock()
            browser.navigate = AsyncMock()
            browser.get_current_url = AsyncMock(return_value=f"https://example{job_id}.com")
            browser.scroll_page = AsyncMock()
            browser.click_element = AsyncMock(return_value=True)
            ...
            await executor.execute_demo(f"https://example{job_id}.com", browser, discovery, planner)
