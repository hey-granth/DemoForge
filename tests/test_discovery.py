...
class TestInteractionDiscovery:
    ...
    async def test_scan_buttons(self, mock_page):
        ...
        mock_button.get_attribute = AsyncMock(side_effect=lambda x: None if x == "id" else "button")
    ...
    async def test_scan_filters_javascript_links(self, mock_page):
        ...
        mock_link.get_attribute = AsyncMock(side_effect=lambda x: "javascript:void(0)" if x == "href" else "link")
        mock_link.inner_text = AsyncMock(return_value="Link")
        
        mock_page.query_selector_all = AsyncMock(side_effect=lambda sel: [mock_link] if "a[href]" in sel else [])
    ...
    async def test_deduplication(self, mock_page):
        ...
        mock_button1.get_attribute = AsyncMock(side_effect=lambda x: None if x == "id" else "button")
