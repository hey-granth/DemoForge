...
class TestInteractionPlanner:
    ...
    async def test_respects_max_actions_limit(self):
        ...
        elements = [{"selector": f"btn{i}", "text": f"Button {i}", "tag": "button"} for i in range(20)]
        
        result = await planner.rank_interactions(elements, "https://example.com", max_actions=3)
    ...
    async def test_gemini_response_parsing(self, mock_genai):
        ...
        mock_response.text = '[{"selector":"btn1","action":"click","priority":1,"reason":"test"}]'
