...
class TestExecutionController:
    @pytest.mark.asyncio
    async def test_enforces_max_clicks(
        self, mock_browser, mock_discovery, mock_planner
    ):
        ...
        await executor.execute_demo(
            "https://example.com", mock_browser, mock_discovery, mock_planner
        )
    ...
    @pytest.mark.asyncio
    async def test_aborts_on_domain_change(
        self, mock_browser, mock_discovery, mock_planner
    ):
        executor = ExecutionController(max_clicks=100, max_depth=10, max_runtime=300)

        elem = InteractionElement(
            "link", "a", "External", "link", "https://other.com", True, True
        )
        mock_discovery.scan_elements = AsyncMock(return_value=[elem])
        mock_discovery.filter_unvisited = MagicMock(return_value=[elem])
        ...
        mock_browser.get_current_url = AsyncMock(
            side_effect=["https://example.com", "https://other.com"]
        )
    ...
    @pytest.mark.asyncio
    async def test_aborts_on_auth_page(
        self, mock_browser, mock_discovery, mock_planner
    ):
        executor = ExecutionController(max_clicks=100, max_depth=10, max_runtime=300)
        ...
        mock_browser.get_current_url = AsyncMock(
            side_effect=["https://example.com", "https://example.com/login"]
        )
    ...
    @pytest.mark.asyncio
    async def test_stops_when_no_unvisited_elements(
        self, mock_browser, mock_discovery, mock_planner
    ):
        executor = ExecutionController(max_clicks=100, max_depth=10, max_runtime=300)
    ...
    @pytest.mark.asyncio
    async def test_tracks_visited_urls(
        self, mock_browser, mock_discovery, mock_planner
    ):
