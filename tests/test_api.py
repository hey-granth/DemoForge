...
class TestAPIEndpoints:
    ...
    def test_get_status_found(self, mock_redis_client, client, sample_job_data):
        mock_redis_client.get = AsyncMock(return_value=json.dumps(sample_job_data).encode())
    ...
    @patch('api.main.redis_client')
    def test_export_video_job_not_complete(self, mock_redis_client, client, sample_job_data):
        incomplete_job = sample_job_data.copy()
        incomplete_job["status"] = "processing"
        mock_redis_client.get = AsyncMock(return_value=json.dumps(incomplete_job).encode())
    ...
    def test_export_video_success(self, mock_redis_client, client, sample_job_data):
        ...
        mock_redis_client.get = AsyncMock(return_value=json.dumps(complete_job).encode())
