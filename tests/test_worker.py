...
class TestDemoWorker:
    ...
    async def test_process_job_updates_status(self, mock_redis, sample_job_data):
        ...
        with patch.object(worker, '_run_demo', AsyncMock(return_value=Path("/tmp/video.mp4"))):
    ...
    @pytest.mark.asyncio
    async def test_process_job_handles_execution_failure(self, mock_redis, sample_job_data):
        worker = DemoWorker()
        worker.redis_client = mock_redis
        
        mock_redis.get.return_value = json.dumps(sample_job_data).encode()
        
        with patch.object(worker, '_run_demo', AsyncMock(side_effect=Exception("Execution failed"))):
            await worker.process_job("test-job-123")
    ...
    async def test_run_demo_initializes_components(self, tmp_path):
        ...
        with patch('worker.runner.BrowserSession') as mock_browser_class:
            ...
            mock_browser.get_video_path = AsyncMock(return_value=tmp_path / "video.webm")
            ...
            with patch('worker.runner.ExecutionController') as mock_executor_class:
                ...
                with patch('worker.runner.VideoProcessor') as mock_processor_class:
                    ...
                    mock_processor.process_video = MagicMock(return_value=tmp_path / "output.mp4")
    ...
    async def test_run_demo_cleans_up_on_failure(self, tmp_path):
        ...
        with patch('worker.runner.BrowserSession') as mock_browser_class:
            ...
            with patch('worker.runner.ExecutionController') as mock_executor_class:
                mock_executor = AsyncMock()
                mock_executor.execute_demo = AsyncMock(side_effect=Exception("Test error"))
                mock_executor_class.return_value = mock_executor
            ...
            with patch('worker.runner.BrowserSession') as mock_browser_class:
                ...
                with patch('worker.runner.ExecutionController') as mock_executor_class:
                    ...
                    with patch('worker.runner.VideoProcessor') as mock_processor_class:
                        ...
                        mock_processor.process_video = MagicMock(return_value=output_path)
                        mock_processor_class.return_value = mock_processor
